...
// Omitting for_date uses today's event date derived from the timezone in config.json.
const scans = [
...
db.serialize(() => {
  ...
  scans.forEach((scan) => {
    const forDate = scan.for_date || eventDate;
    db.run(
      `INSERT OR IGNORE INTO scans (user_id, tag_id, for_date) VALUES (?, ?, ?)`,
      [scan.user_id, scan.tag_id, forDate],
      (err) => {
