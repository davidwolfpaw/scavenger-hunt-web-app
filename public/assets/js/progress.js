...
async function handleProgressPage() {
  ...
  try {
    ...
    if (data.success) {
      ...
      const completionRes = await fetch(`/user/${identifier}/completion`);
      const completionData = await completionRes.json();

      if (completionData.success && completionData.completed) {
        const congratsMessage = document.createElement('p');
        congratsMessage.textContent = "Congratulations on completing the scavenger hunt!";
        congratsMessage.style.fontWeight = 'bold';
        congratsMessage.style.color = 'green';

        const qrCodeImage = document.createElement('img');
        qrCodeImage.src = completionData.qrCode;
        qrCodeImage.alt = "Completion QR Code";
        qrCodeImage.style.display = 'block';
        qrCodeImage.style.marginTop = '20px';

        scanGrid.appendChild(congratsMessage);
        scanGrid.appendChild(qrCodeImage);
      }
    } else {
